    ...
    <li className="text-left mb-10">
      <div className="flex flex-row items-start">
          <div className="flex flex-col items-center justify-center mr-5">
              <div
                  className={cn("flex items-center justify-center size-8 rounded-full text-white border-4 border-white text-md font-semibold", result && statusColors(result[0].label) + " animate-bounce ring")}>
                  {loading ? 
                  <div role="status">
              ...
              <div className="mt-4 text-gray-500">
                <span className={cn("inline-flex items-center rounded-xl px-2 py-1 text-xs font-medium", !loading2 && result2 && toneColors(result2[0].label))}>{loading2 ? "LOADING" : result2 && result2[0].label}</span>
              </div>
              {/* <span className="text-gray-500 mt-2">Predicted Ratings</span> */}
          </div>
          <div className="bg-gray-100 px-4 py-3 rounded-xl w-full">
              <h4 className="text-lg leading-6 font-semibold text-gray-900">{header}</h4>
